...
// 使用统一的MarketData定义替代重复的MarketDataInput
pub use common_types::{MarketData as MarketDataInput};
...
impl OptimizedArbitrageEngine {
    ...
    pub async fn process_market_data(&self, data: MarketDataInput) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 转换为固定点数格式 - 使用common_types::MarketData字段
        let buy_price = FixedPrice::from_f64(data.ask_price); // 买入价格是ask
        let sell_price = FixedPrice::from_f64(data.bid_price); // 卖出价格是bid
